...
@workflow(name="rce", type="fuzzing", severity="critical", confidence="confirmed")
def execute(scan_type, mode, target, config, scan_id):
    print(f'{Fore.GREEN}[INFO]{Fore.YELLOW} Rce_fuzz')
    ...
    ct = Create_template() \
        .template(PWD + "/src/test/resources/templates/nuclei/fuzzing/cmdi_blind_fuzz.yaml") \
        .working_dir(config['execution_dir']) \
        .output("cmdi_fuzz") \
        .requests(rr) \
        .execute().results[0]
    ct = Create_template() \
        .template(PWD + "/src/test/resources/templates/nuclei/fuzzing/cmdi_fuzz.yaml") \
        .working_dir(config['execution_dir']) \
        .output("cmdi_fuzz") \
        .requests(rr) \
        .execute().results[0]
    ...
    nuclei = Nuclei() \
        ...
        .markdown_export(create_valid_path(config['execution_dir'], "result_cmd_fuzz")) \
