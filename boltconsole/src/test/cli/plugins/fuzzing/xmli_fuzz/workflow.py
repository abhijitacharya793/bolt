...
@workflow(name="xmli", type="fuzzing", severity="critical", confidence="not_confirmed")
def execute(scan_type, mode, target, config, scan_id):
    print(f'{Fore.GREEN}[INFO]{Fore.YELLOW} XMLI_fuzz')
    ...
    ct = Create_template() \
        .template(PWD + "/src/test/resources/templates/nuclei/fuzzing/xmli_fuzz.yaml") \
        .working_dir(config['execution_dir']) \
        .output("xmli_fuzz") \
        .requests(rr) \
        .execute().results[0]
    ...
    nuclei = Nuclei() \
        ...
        .markdown_export(create_valid_path(config['execution_dir'], "result_xmli_fuzz")) \
